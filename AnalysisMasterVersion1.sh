...
Fastqc_root="/home/ubuntu/programs/FastQC/" ; #can be installed globally (comment out); otherwise, added to PATH on next line
export PATH=""${Fastqc_root}":$PATH" ;
#Cutadapt_root="/home/ubuntu/programs/cutadapt-1.14/" ; #installed globally - used by TrimGalore
...
echo "Beginning analysis script on `date`, run by "${User}" with the following parameters:" \
	> "${Results_root}"/"${RunNumber}"/"${PatientID}"/"${PatientID}"_AnalysisLog.txt ;
echo -e "\t1\t"${Read1}"" \
	>> "${Results_root}"/"${RunNumber}"/"${PatientID}"/"${PatientID}"_AnalysisLog.txt ;
echo -e "\t2\t"${Read2}"" \
	>> "${Results_root}"/"${RunNumber}"/"${PatientID}"/"${PatientID}"_AnalysisLog.txt ;
...
echo `"${TrimGalore_root}"/trim_galore \
...
echo `"${SAMtools_root}"samtools view -bS \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned.sam \
	> "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned.bam` ;

echo `"${SAMtools_root}"samtools sort \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned.bam \
...
echo `"${SAMtools_root}"samtools view \
...
echo `"${SAMtools_root}"samtools flagstat \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bam \
	> "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Alignment.txt` ;

echo `"${BEDTools_root}"bedtools intersect \
	-v -bed \
...
echo `"${BEDTools_root}"bedtools genomecov \
...
echo `"${BEDTools_root}"bedtools coverage \
	-a "${BEDfile}" \
	-b "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bam \
	-d \
	> "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_PerBaseDepthBED.bedgraph` ;
...
zero=$("${BEDTools_root}"bedtools genomecov \
	-ibam "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bam \
	-g "${Ref_FASTA}" \
	-bga | \
	awk '$4==0 {bpCountZero+=($3-$2)} {print bpCountZero}' | \
	tail -1)
nonzero=$("${BEDTools_root}"bedtools genomecov \
	-ibam "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bam \
	-g "${Ref_FASTA}" \
	-bga | \
...
echo `java -jar "${Picard_root}"picard.jar DownsampleSam \
...
then
	echo `"${BCFtools_root}"bcftools mpileup \
	--redo-BAQ \
	--min-BQ 30 \
	--per-sample-mF \
...
#Re-header the VCF
echo -e ""${PatientID}"" \
	> "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/samples.tsv ;
...
echo -e ""${PatientID}"_25pc" \
	>> "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/samples.tsv ;
echo `"${BCFtools_root}"bcftools reheader \
	--samples "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/samples.tsv \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bcf \
...
echo `"${BCFtools_root}"bcftools norm \
	-Ou \
	-m-any \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bcf \
	| "${BCFtools_root}"bcftools norm \
		-Ov \
		-f "${Ref_FASTA}" \
...
echo `"${BCFtools_root}"bcftools view \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.vcf \
	| "${BCFtools_root}"misc/vcfutils.pl varFilter \
...
#Sort the VCF
echo `"${BCFtools_root}"bcftools sort -Ov \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ_FiltExtra.vcf \
...
echo `"${VEP_root}"vep \
	...
	--output_file "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_AnnotationVEP.tsv \
	--tab \
...
#Copy analysis files
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bam \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped_FiltMAPQ.bam.bai \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Aligned_Sorted_PCRDuped.txt \
	"${Results_root}"/"${RunNumber}"/"${PatientID}"/"${PatientID}"_PCRDuplicates.txt ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Alignment.txt \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_ReadsOffTarget.txt \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_CoverageTotal.bedgraph \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_PerBaseDepthBED.bedgraph \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_MeanCoverageBED.bedgraph \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_PercentGenomeCovered.txt \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_Final.vcf \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_AnnotationVEP.tsv \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
cp "${Results_root}"/"${RunNumber}"/"${PatientID}"/tmp/"${PatientID}"_AnnotationVEP.html \
	"${Results_root}"/"${RunNumber}"/"${PatientID}" ;
